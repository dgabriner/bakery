...
echo "\n=== Display modes ===\n";
fu_assert_true(bakery_formula_unit_modes(true) === ['g', 'lb', 'gal'], 'baker modes are g/lb/gal');
fu_assert_true(bakery_formula_default_unit_mode(true) === 'g', 'baker default is grams');
fu_assert_true(bakery_formula_normalize_unit_mode('all', true) === 'g', 'saved All does not leak onto baker view');
fu_assert_true(bakery_formula_normalize_unit_mode('lb', true) === 'lb', 'baker can keep pounds');
fu_assert_true(in_array('all', bakery_formula_unit_modes(false), true), 'ops view still has All');
