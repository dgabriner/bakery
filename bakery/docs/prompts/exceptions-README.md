# Exception workability prompts

Paste-ready Cursor briefs for three parallel upgrade missions. Open each in its own chat in `bakery/`. All three may start the same day if they obey file ownership.
...
Read first: `BAKERY_PRODUCT_CONTEXT.md` §§3, 6, 8, 10.
Canonical exception contract: `includes/operational_exceptions.php`.
Work queue: `includes/manager_mode.php` + `manager_exception_work`.
Failed stops: `includes/delivery_recovery.php` + `docs/FAILED_STOP_RECOVERY_MODEL.md`.
