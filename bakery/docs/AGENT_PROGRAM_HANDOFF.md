...
| 60 `overnight-cron` — stamps + stale chip + runbook | **shipped (agent)** | `cron_run` + `storage/cron`; health age_hours; dashboard >26h; CRON_KIT (owner installs crontab) |
| 64 | **owner-decision** | retail cashier scope |
...
9. ~~**62**~~ shipped. ~~**61 steps 1–3**~~ partial. ~~**63 step A**~~ partial. ~~**60 agent parts**~~ shipped (owner installs crontab). **64** owner.
...
8. **Next agent / owner:** Install DreamHost crontab from `docs/CRON_KIT.md`; pin remaining Wave 4 Decideds (61 ledger, 63B stock adjust, 64 retail). **Wave 4 no-decision agent work is done.** Staging and Live were not touched.
