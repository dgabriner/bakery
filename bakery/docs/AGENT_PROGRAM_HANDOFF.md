...
| 52–55, 60–64 | **briefed, not built** | `docs/prompts/NN-*.md` + `includes/agent_program_map.php` |
...
8. ~~**50 `extract-assets`**~~ — **shipped**. ~~**51 `split-actions`**~~ — **shipped** (daily_orders, standing_orders_manager, driver_assignment, production_center). Then **52 `one-mutation-path`** → **53 `hot-path-queries`** → **54 `gate-scaling`** → **55 `product-boundaries`**.
...
8. **Next agent:** Wave 3 continues: **52 `one-mutation-path`** → 53 → 54 → 55. Then Wave 4 no-decision parts. **35–37, 40–46, 50–51 are shipped.** Staging and Live were not touched.
