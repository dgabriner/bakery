...
// Rolling horizon from today, plus this screen's date if it sits outside.
// Always preserves standing→dated quantity edits.
$autoGeneratedDemand = null;
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    try {
        $horizon = bakery_fill_demand_horizon($db, $selectedDate);
        if ((int)($horizon['ran_count'] ?? 0) > 0) {
            $autoGeneratedDemand = $horizon;
        }
    } catch (Throwable $e) {
