...
$stats = [
    ...
    'confirmed' => 0, 'needs_attention' => 0, 'ready' => 0, 'invoiced' => 0,
];
    ...
    <div class="invoice-center-layout">
        <section class="invoice-list-panel">
            ...
            <?php else: ?>
                ...
                $readyCount = 0;
                foreach ($orders as $order) {
                    if ($order['delivery_confirmed_at'] && $order['status'] !== 'invoiced') {
                        $readyCount++;
                    }
                }
                ?>
                <form method="post" action="billing_api.php" id="bulkInvoiceForm" onsubmit="return confirm('Mark all selected deliveries as invoiced?');">
                    <?php echo function_exists('bakery_csrf_field') ? bakery_csrf_field() : ''; ?>
                    <input type="hidden" name="action" value="bulk_mark_invoiced">
                    <input type="hidden" name="redirect" value="<?php echo htmlspecialchars($query(['invoice_id' => null])); ?>">
                    <?php if ($readyCount > 0): ?>
                        <div class="ic-bulk-bar">
                            <label class="ic-bulk-select-all"><input type="checkbox" id="bulkSelectAll"> Select ready (<strong><?php echo $readyCount; ?></strong>)</label>
                            <button class="ic-btn ic-btn-primary" type="submit" id="bulkInvoiceSubmit" disabled>Mark invoiced (<span id="bulkInvoiceCount">0</span>)</button>
                        </div>
                    <?php endif; ?>
                ...
                <?php foreach ($groupedOrders as $groupLabel => $groupOrders): ?>
                    ...
                    <?php foreach ($groupOrders as $order): ?>
                        <?php $orderReady = $order['delivery_confirmed_at'] && $order['status'] !== 'invoiced'; ?>
                ...
                <article class="invoice-detail">
                    ...
                    <div class="invoice-detail-footer">
                        <?php if ($selectedInvoice['delivery_confirmed_at'] && $selectedInvoice['status'] !== 'invoiced'): ?>
                            <form method="post" action="billing_api.php" style="display:inline" onsubmit="return confirm('Mark invoiced?');">
                                <?php echo function_exists('bakery_csrf_field') ? bakery_csrf_field() : ''; ?>
                                ...
                                <button class="ic-btn ic-btn-primary" type="submit">Mark invoiced</button>
                            </form>
                        <?php endif; ?>
                    ...
                    <p class="ic-note">Line prices from <code>daily_order_items.unit_price</code> snapshot at order time. Billable total from delivery confirmation snapshot — not live catalog pricing.</p>
                </article>
            <?php else: ?>
