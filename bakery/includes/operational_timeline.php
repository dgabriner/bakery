...
function bakery_operational_event_category(string $eventType): string
{
    static $map = [
        ...
        BAKERY_OP_PRODUCTION_ASSIGNED => 'production',
        BAKERY_OP_PRODUCTION_CUT => 'production',
...
function bakery_operational_timeline_links(array $entry): array
{
    ...
    if (($entry['event_type'] ?? '') === BAKERY_OP_PRODUCTION_PLAN_SAVED
        ...
        || ($entry['event_type'] ?? '') === BAKERY_OP_PRODUCTION_ASSIGNED
        || strpos((string)($entry['event_type'] ?? ''), 'inventory_production') === 0) {
        $links['production'] = (($entry['event_type'] ?? '') === BAKERY_OP_PRODUCTION_ASSIGNED)
            ? ('production_center.php?date=' . urlencode((string)($entry['operational_date'] ?? date('Y-m-d'))))
            : ('production.php?date=' . urlencode((string)($entry['operational_date'] ?? date('Y-m-d'))));
    }
    return $links;
}
