...
/**
 * Allowed formula unit-display modes.
 *
 * Baker work view is g / lb / gal only. Ops may also show all three at once.
 *
 * @return list<string>
 */
function bakery_formula_unit_modes($bakerWorkView = false)
{
    if ($bakerWorkView) {
        return ['g', 'lb', 'gal'];
    }
...
function bakery_formula_default_unit_mode($bakerWorkView = false)
{
    return $bakerWorkView ? 'g' : 'all';
}

function bakery_formula_normalize_unit_mode($mode, $bakerWorkView = false)
{
    $mode = (string) $mode;
    $allowed = bakery_formula_unit_modes($bakerWorkView);
    if (in_array($mode, $allowed, true)) {
        return $mode;
    }
    return bakery_formula_default_unit_mode($bakerWorkView);
}
