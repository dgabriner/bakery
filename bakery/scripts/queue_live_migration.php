<?php
/**
 * CLI helper on hosted Staging: approve the next additive migration(s) for Live.
 */
...
foreach ($argv as $arg) {
    if (strpos($arg, '--file=') === 0) {
        $targetFile = basename(substr($arg, 7));
    }
}

if (!defined('IS_STAGING') || !IS_STAGING) {
    fwrite(STDERR, "Live migration approval is staging-only.\n");
    exit(1);
}
...
try {
    $result = bakery_hosted_migration_approve_recommended($db, $targetFile);
