-- 053 — Safe Live promotion for the already-tested 052 pack-yield reference.
-- superseded-by: 054_live_product_pack_yields_mysql_compat
--
-- 052 uses ON DUPLICATE KEY UPDATE so Staging can revise reference data. The
-- hosted Live gate intentionally refuses that broad update form. Live does not
-- yet have these tables, so this forward migration creates the same structure
-- and inserts only missing reference rows. It never changes existing Live data.
-- This file is also the immutable source exported by the Staging approval gate.
-- Its initial reference rows deliberately preserve existing Live overrides.
...
-- Concha + Fino: 3 gal → 44 trays → 880 pieces.
